...
abstract public class MenuTransform implements MenuTransformable {
	...
	public void processPixels(SerializablePicture source1, SerializablePicture source2, SerializablePicture target, ComponentFilter cf) {
		processPixels(source1, source2, target, makeColorFilter(cf));
	}
	
	private static Color getColor(SerializablePicture p, int x, int y) {
	 ...
	 */
	private static SerializablePicture scalePicture(SerializablePicture in, SerializablePicture standard) {
		double w = in.width();
		double h = in.height();
		SerializablePicture ans = new SerializablePicture(standard.width(), standard.height());
	...
	public void processPixels(SerializablePicture source1, SerializablePicture source2, SerializablePicture target, ColorFilter ccf) {
	...
	public void processPixels(SerializablePicture source1, SerializablePicture source2,
			SerializablePicture target, PictureFilter pf) {
		pf.filter(new SerializablePicture[] { scalePicture(source1,target), target });
